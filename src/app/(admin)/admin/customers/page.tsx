...
const AdminCustomersPage = async ({
  ...
  return (
    <Stack gap={3}>
      <AdminPageHero
        eyebrow={tCustomers("eyebrow")}
        title={tCustomers("title")}
        description={tCustomers("description")}
      />

      {deleted === "1" ? (
        <Alert severity="success">{tCustomers("deletedMessage")}</Alert>
      ) : null}
      {error ? (
        <Alert severity="error">
          {getAdminCustomerFormErrorMessage(error, customerFormErrorMessages)}
        </Alert>
      ) : null}

      <AdminSectionCard
        title={tCustomers("sectionTitle")}
        description={`${tCustomers("sectionDescription")}: ${customers.length}`}
      >
            ...
            <TableBody>
              {customers.map((customer) => (
                <TableRow key={customer.id} hover>
                  ...
                  <TableCell>
                    {customer.phone || sharedPlaceholders.emptyValue}
                  </TableCell>
                  <TableCell>
                    <AdminStatusChip
                      label={
                  ...
                  <TableCell align="right">
                    <Stack direction="row" gap={0.5} justifyContent="flex-end">
                      ...
                      <DeleteCustomerButton
                        ...
                        icon={
                          <DeleteOutlineOutlinedIcon key="delete-customer-icon" />
                        }
                        iconOnly
                        size="small"
